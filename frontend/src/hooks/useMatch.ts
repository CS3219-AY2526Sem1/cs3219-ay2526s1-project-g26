...
import { setLastCollaborationId } from '../store/slices/navbarSlice.ts'
import { setCurrentQuestion } from '../store/slices/collaborationSlice.ts'
    ...
    (checkedTopics: string[], checkedDifficulties: string[]) => {
      try {
        ...
        socket.on('matchSuccess', (res) => {
          ...
          cleanup()
          dispatch(setLastCollaborationId(roomid))
